...
export function ActivityView() {
  ...
  return (
    <div className="absolute inset-0 flex flex-col items-center justify-center gap-1.5 bg-screen/95 px-[19%] text-center">
      <p className="w-full truncate text-[10px] font-bold uppercase tracking-[0.18em] text-cream-200/50">
        {activity.title}
      </p>

      {activity.phase === 'error' ? (
        <p className="text-[11px] font-semibold leading-snug text-berry-500">{activity.error}</p>
      ) : (
        <>
          {activity.caption && (
          ...
          {status && <p className="text-[10px] font-medium text-cream-200/50">{status}</p>}
        </>
      )}
